...
function main() {
    ...
    if [ ! -d ${xkb_path} ]; then
        echo "There is no directory \"${xkb_path}\". Open util.sh and set XKB path."
        exit 0
    fi
...
function add_variant() {
    local rule_name=$1
    local format=$2
    local rule_file="${xkb_path}/rules/${rule_name}.${format}"
    ...
    if [ -n "${desc_ln_in_rule_file}" ]; then
        ...
        if [ -n ${exist_ln} ]; then
            ...
            { head -n $((exist_ln-desc_ln_in_variant)) "${bak_file}"; \
              sed "s/@n/${variant_name}/g" "${variant_file}" | sed "s/@d/${variant_desc}/g"; \
...
function add_keymap() {
    local keymaps_file="${xkb_path}/symbols/${country}"
    local ln_variant_desc=$(get_line_containing "${variant_desc}" "${keymaps_file}")
    
    if [ -n "${ln_variant_desc}" ]; then
        echo "There is already \"${variant_desc}\" keymap in \"${keymaps_file}\""
    else
        printf "Adding \"${variant_desc}\" keymap to \"${keymaps_file}\"..."

        local bak_file=$(get_backup_file "${bak_subdir}" "${keymaps_file}")
        cp "${keymaps_file}" "${bak_file}"

        { echo "// ${section_start}"; cat "./keymap"; echo "// ${section_end}"; } >> "${keymaps_file}"
...
# In the future Compose file can be placed at /usr/share/X11/locale/hy_AM.UTF-8/
